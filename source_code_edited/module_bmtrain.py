...
def load_plm(model_name, model_path, specials_to_add = None,dtype=torch.float16):
    ...
    if model_path == "roberta-base":
        model_path = '/home/zhujiangyi/.cache/model_center/roberta-base'
    if model_path == "roberta-large":
        model_path = '/home/zhujiangyi/.cache/model_center/roberta-large'
    ...
    model = model_class.model.from_pretrained(model_path, config=model_config,return_logits=True)
    # if dtype == 'float16':
    #     print(model_path+'/pytorch_model.pt')
    #     bmt.load(model,model_path+'/pytorch_model.pt')
...
class PromptModel(nn.Module):
    ...
    def forward(self, batch: Union[Dict, InputFeatures]) -> torch.Tensor:
        ...
        outputs = self.plm(**input_batch, return_logits=True)
