...
class RoertaPooler(torch.nn.Module):
    def __init__(self, dim_model,dtype=torch.float16):
        super().__init__()
...
class RoertaLMHead(torch.nn.Module):
    def __init__(self, dim_model, vocab_size, norm_eps,dtype=torch.float16):
        super().__init__()
        self.dense = Linear(dim_model, dim_model, bias=True,dtype=dtype)
...
class Roberta(BaseModel):
    ...
    def __init__(self, config: RobertaConfig):
        ...
        self.pooler = RoertaPooler(config.dim_model,dtype=config.dtype)
    ...
    def forward(self,
        ...
        with torch.no_grad():
            ...
            if attention_mask is not None:
                ...
                length = torch.tensor([batch], device=device) ############这里后加                
