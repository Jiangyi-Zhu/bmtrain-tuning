...
class MixedTemplate(Template):
    ...
    def __init__(self,
        ...
        self.raw_embedding = model.get_input_embeddings()###########改了这里
        #self.raw_embedding = model.input_embedding
    ...
    def prepare(self):
        ...
        **Note that ``"soft_id"`` should have index start from 1 but not 0**
        ...
        # self.soft_embedding = model_center.layer.Embedding(1 + self.num_soft_token, self.embedding_size,init_mean=0,init_std=5)
        # print(self.soft_embedding.weight.data)
        dtype=self.raw_embedding.weight.data.dtype
        self.soft_embedding = nn.Embedding(1 + self.num_soft_token, self.embedding_size,dtype=dtype)
        # a=torch.load('/home/zhujiangyi/prompt_learning/embedding')
        # self.soft_embedding.weight.data=a
        # print(self.soft_embedding.weight.data)
        for soft_id, token_id in emb_mp.items():
            self.soft_embedding.weight.data[soft_id, :] = self.raw_embedding.weight.data[token_id, :].clone().detach().requires_grad_(True).half()
